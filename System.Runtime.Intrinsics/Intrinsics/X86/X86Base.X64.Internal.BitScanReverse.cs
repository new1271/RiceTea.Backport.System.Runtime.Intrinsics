...
partial class X86Base
{
    unsafe partial class X64
    {
        ...
        private static void InjectBsrAsm_Unix(ref void* destination, ref uint length)
        {
            ...
            if (!_spanExists)
            {
                UnsafeHelper.CopyBlock(destination, in StoreAsArray.GetBsrDataReference_Unix(), Length);
                return;
            }
#endif
            UnsafeHelper.CopyBlock(destination, in StoreAsSpan.GetBsrDataReference_Unix(), Length);
        }
