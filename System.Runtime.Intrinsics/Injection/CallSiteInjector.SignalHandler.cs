    ...
    private static unsafe class SignalHandler
    {
        ...
        static SignalHandler()
        {
            ...
            *lastHandlerSlot = MemoryHelper.GetFuncAddress_OnlyUnix("abort");
            (NativeFunctionHolder holder, nuint slotOffset) = SoftDependencyHelper.SystemMemoryExists ?
                StoreAsSpan.BuildSignalHandlerAsm() :
                StoreAsArray.BuildSignalHandlerAsm();
        ...
        private static class StoreAsArray
        {
            ...
            private static (NativeFunctionHolder Holder, nuint SlotOffset) BuildSignalHandlerAsm_X86()
            {
                ...
                byte[] data = new byte[Length] // Original from SignalHook project in the solution
                {
        ...
        private static class StoreAsSpan
        {
            ...
            {
                const int Length = 113;
                ReadOnlySpan<byte> data = // Original from SignalHook project in the solution
                [
                    0x53, 0x56, 0x50, 0xE8, 0x00, 0x00, 0x00, 0x00,
                    0x5B, 0x81, 0xC3, 0x03, 0x00, 0x00, 0x00, 0x8B,
                    0x44, 0x24, 0x14, 0x8B, 0x4C, 0x24, 0x10, 0x83,
